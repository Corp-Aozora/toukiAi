...
class CustomMessageTemplates extends MessageTemplates{
    ...
    static someInvalidInputWhenLoad(instance){
        return `${UpdateTitle.getFieldsetTitle({instance: instance})}に入力不備があるため次へボタンが有効化されてません`;
    }
...
class UpdateTitle{
    ...
    static childOrCollateralFollowing(titleEl, zokugara){
        ...
        // タイトルの[本番－枝番]の部分を取得する
        function getTitleNumbering(){
            const s = titleEl.textContent;
            return s.replace(/\n|\s/g, "").split("．")[0];
        }

        const oldNumbering = getTitleNumbering(); //旧ナンバリングを取得する
...
/**
 * 値を代入してchangeイベントを発火する
 * @param {string|number|boolean} val 
 * @param {HTMLInputElement} input
 */
function assignAndDispatchChangeEvent(val, input){
    if(val === null)
        return;
 ...
 * @returns 
 */
function isBooleanData(data, key){
    const targetData = data[key];
    if(typeof targetData !== "boolean")
 ...
 */
function toggleAssignAndDispatchChangeEvent(data, yesInput, noInput){
    const isDisabled = yesInput.disabled;
    if(isDisabled)
        return;

    assignAndDispatchChangeEvent(true, data? yesInput: noInput);
}

/**
 * ユーザーのデータを取得する
 * 
 * 入力によって初期化される欄があるため手動入力とイベント発生
 */
async function loadData(){
    ...
    // 特定の欄を復元
    function loadQData(data, key, inputs, idxs){
        const [yesInput, noInput] = [inputs[idxs[yes]], inputs[idxs[no]]];
        const result = isBooleanData(data, key);
        toggleAssignAndDispatchChangeEvent(result, yesInput, noInput);
        return result;
    }

    // 配偶者または尊属の相続放棄していないときの処理
    function handleSpouseOrAscendantIsRefuseFalse(isLive, data, instance){
        const {inputs, constructor} = instance;
    ...
    // 直系卑属または兄弟姉妹が手続時に生存しているときの処理
    function handleDescendantOrCollateralIsLiveTrue(data, instance){
        const {inputs, constructor} = instance;
        const idxs = constructor.idxs;
        ...
        result? handleIsRefuseTrue(instance): handleIsRefuseFalse();
    }

    // 配偶者（子の配偶者を含む）の処理
    function handleSpouseEvent(data, instance){
        const functionName = "handleSpouseEvent";
    ...
    // 子共通または兄弟姉妹共通フォームの処理
    function loadCommonQ(count, inputs, countInputIdx){
        const functionName = "loadCommonQ";
    ...
    // 孫または兄弟姉妹の手続時生存確認欄の処理
    function handleGrandChildOrCollateralIsLive(data, instance){

        const {inputs, constructor} = instance;
        const idxs = constructor.idxs;

        // 手続時に死亡しているとき
        function handleIsLiveFalse(){
            
            // 相続放棄欄の処理
            function handleIsExistTrue(){
                ...
                const result = isBooleanData(data, "is_refuse");
                result? handleIsRefuseTrue(instance): handeIsRefuseFalse();
            }
     ...
     */
    async function loadDecedent(){
        const functionName = "loadDecedent";
        ...
        await dispatchNextBtnEvent(decedent);
    }

    /**
     * 被相続人の配偶者のデータをロード
     */
    async function loadDecedentSpouse(){
        handleSpouseEvent(spouse_data, spouse);
        await dispatchNextBtnEvent(spouse);
    }

    /**
     * 子供共通のデータロード
     */
    async function loadChildCommon(){
        ...
        loadCommonQ(count, inputs, countInputIdx);

        // 同じ両親確認欄がいいえ、かつ子が１人、かつ配偶者がいないとき健在確認欄を非表示にする
        ...
        await dispatchNextBtnEvent(childCommon);
    }
    ...
    async function loadChild(){
        ...
        function handleIsSameParents(objectId2Data, yesInput, noInput){
            const isSameParentsYes = objectId2Data === spouse_data["id"];
            toggleAssignAndDispatchChangeEvent(isSameParentsYes, yesInput, noInput);
        }

        // 手続時生存欄の処理
        function handleIsLive(data, inputs, instance){
            ...
            function handleIsLiveFalse(){
                ...
                function handleIsChild(){
                    ...
                    const isChild = count > 0;
                    assignAndDispatchChangeEvent(true, inputs[CIsChild.input[isChild? yes: no]]);
                    if(isChild)
                        inputs[CCount.input].value = count;
                }

                // 相続放棄欄の処理
                function handleIsRefuse(){

                    // 子の配偶者確認欄の処理
                    function handleIsSpouse(){
                        const isSpouse = data["is_spouse"] > 0;
                        assignAndDispatchChangeEvent(true, inputs[CIsSpouse.input[isSpouse? yes: no]]);
                    }
                    
                    // 相続放棄していないときは子の配偶者と子の子の存在確認
                    const result = loadQData(data, "is_refuse", inputs, CIsRefuse.input);
                    if(!result){
                        handleIsSpouse();
                        handleIsChild();
                    }
                ...
                const result = loadQData(data, "is_exist", inputs, CIsExist.input);
                result? handleIsRefuse(): handleIsChild();
            }

            const result = loadQData(data, "is_live", inputs, CIsLive.input);
            result? handleDescendantOrCollateralIsLiveTrue(data, instance): handleIsLiveFalse();
        }

        const functionName = "loadChild";

        for(let i = 0, len = childs.length; i < len; i++){
            const instance = childs[i];
            const {inputs} = instance; 
            const data = childs_data[i];
            try{
                handleName(data, instance);
                handleIsSameParents(data["object_id2"], inputs[CIsSameParents.input[yes]], inputs[CIsSameParents.input[no]]);
                handleIsLive(data, inputs, instance);

                await dispatchNextBtnEvent(instance);
            }catch(e){
                throw new Error(CustomMessageTemplates.errorWhenLoad(functionName, instance, e));
            }
        }
     ...
     * 子の相続人のデータロード
     */
    async function loadChildHeir(){
        const functionName = "loadChildHeir";
        const instances = getSortedChildsHeirsInstance();

        // 孫の処理
        function handleGrandChilds(instance, inputs, data){

            // 同じ両親確認欄の処理
            function handleIsSameParents(){
                const isSameParents = data["content_type2"] && data["content_type2"] == 9;
                assignAndDispatchChangeEvent(true, inputs[GIsSameParents.input[isSameParents? yes: no]]);
            }

            try{
                handleName(data, instance);
                handleIsSameParents();
                handleGrandChildOrCollateralIsLive(data, instance);
            }catch(e){
                throw new Error(CustomMessageTemplates.errorWhenLoad(functionName, instance, e));
            }
        }
        ...
        for(let i = 0, len = instances.length; i < len; i++){
            ...
            if(constructor.name === "ChildSpouse")
                handleSpouseEvent(data, instance);
            else
                //孫のとき
                handleGrandChilds(instance, inputs, data);
     ...
     * 尊属のデータロード
     */
    async function loadAscendant(){
        
        // 手続時生存欄の処理
        function handleIsLive(isLive, data, inputs, instance){
            ...
            // 相続時存在欄の処理
            function handleIsExist(){
                const result = isBooleanData(data, "is_exist");
                assignAndDispatchChangeEvent(true, inputs[AIsExist.input[result? yes: no]]);
                if(result)
                    handleIsRefuse();
            }

            assignAndDispatchChangeEvent(true, inputs[AIsLive.input[isLive? yes: no]]);
            isLive? handleIsRefuse(): handleIsExist();
        }

        const functionName = "loadAscendant";
        ...
        for(let i = 0; i < ascendants.length; i++){
            ...
            try{
                ...
                const result = isBooleanData(data, "is_live");
                handleIsLive(result, data, inputs, instance);
     ...
     * 兄弟姉妹共通のデータロード
     */
    async function loadCollateralCommon(){
        const instance = collateralCommons[0];
        const {inputs} = instance;
        const count = instance.countCollaterals();

        loadCommonQ(count, inputs, CoCCount.input);
    ...
    async function loadCollateral(){
        ...
        // 同じ両親確認欄の処理
        function handleSameParents(data, inputs){

            const isSameParents = data["content_type1"] === data["content_type2"];
            toggleAssignAndDispatchChangeEvent(isSameParents, inputs[ColIsSameParents.input[yes]], inputs[ColIsSameParents.input[no]]);

            // 異父母がいるとき、父母のいずれかを同じにする
            if(!isSameParents){
                const isSameFather = data["content_type1"] === 5;
                assignAndDispatchChangeEvent(true, inputs[ColIsFather.input[isSameFather? yes: no]]);
            }
        }

        const functionName = "loadCollateral";
    ...
    if(userDataScope.includes("spouse"))
        await loadDecedentSpouse();

    // 子共通
    if(userDataScope.includes("child_common"))
        await loadChildCommon();
...
function validateOldLawInheritance(year, month){
    ...
    // 指定された年月を表す Date オブジェクトを作成
    const targetDate = new Date(intYear, intMonth - 1); // 月は0から11であるため、-1

    // 昭和22年5月（1947年5月1日）の開始日
    const newLawStartDate = new Date(1947, 4, 1); // 月は0から11なので、4は5月

    // 指定された年月が昭和22年5月1日よりも前の場合は false、それ以外は true
    const result = targetDate >= newLawStartDate;
    return result? true: "家督相続には対応してません。";
}

/**
 * 年月の検証ハンドラー
 * 
 * 空欄チェック/ 未来の年月チェック/ 家督相続以降の死亡日チェック
 * @param {HTMLInputElement[]} inputs 
 * @param {HTMLInputElement} input 
 * @returns 
 */
function handleYearAndMonthValidation(inputs, input){
    ...
    result = validateOldLawInheritance(inputs[DDeathYear].value, inputs[DDeathMonth].value);
    if(typeof result === "string")
        return result; 
...
function decedentValidation(inputs, index){
    ...
    const functionName = "decedentValidation";
    ...
    if(index === DName)
        ...
        throw new Error(`${functionName}でエラー発生\ninputs=${inputs}\nindex=${index}`);
}
